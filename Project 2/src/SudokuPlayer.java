...
public class SudokuPlayer implements Runnable, ActionListener {
  ...
  // Useful but not required Data-Structures;
    ...
    *  These will be the data structures necessary for AC-3. 
        **/
        ...
        //System.out.println("This is vals:"+vals.length);
        for(int k=0;k<9;k++){
          values.add(k,k+1);
        }int count=0;
        for(int i=0;i<9;i++){
          for(int j=0;j<9;j++){
                if(vals[i][j]==0){
                  ...
                  System.out.println(globalDomains[count]);
                }else{
    ...
    System.out.println("This is gD's length: "+globalDomains.length);
       ...
       int[] row9 = new int[]{72,73,74,75,76,77,78,79,80};
  ...
  // This is discussed in the book. You may change this method header.
    ...
    // YOUR CODE HERE
       int ibug = 0;
       int jbug = 0;
       for(int i: Arrays.copyOfRange(all, 0, all.length - 2)){
         ibug++;
         for(int j :Arrays.copyOfRange(all, 1, all.length - 1)){
           jbug++;
           if(j!=i){
             Arc a = new Arc(i,j);
             globalQueue.add(a); 
       ...
       }//System.out.ln("This is i: "+ibug+" This is j: "+jbug+"\n");
     }
  ...
  // ( You may change this method header )
    private final boolean Revise(Arc arc, ArrayList<Integer>[] Domains){
        ops += 1;
       // ArrayList<Integer> domainI =(ArrayList<Integer>) Domains[arc.Xi].clone();
        //ArrayList<Integer> domainJ =(ArrayList<Integer>) Domains[arc.Xj].clone();

        boolean revised=false;
        boolean allequal=true;
        Iterator<Integer> iter = Domains[arc.Xi].iterator();
        Iterator<Integer> jter = Domains[arc.Xj].iterator();
